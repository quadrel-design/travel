...
module.exports = async function initializeDatabase(pool) {
  if (!pool) {
    logger.error('Database initialization failed: No valid PostgreSQL pool provided');
  ...
  try {
    ...
    logger.info('Initializing database schema by executing schema.sql...');
    ...
    logger.error('CRITICAL: Error initializing database schema from schema.sql:');
    logger.error('Error Name:', error.name);
    logger.error('Error Message:', error.message);
    logger.error('Error Code:', error.code); // PostgreSQL error code if available
    logger.error('Error Position:', error.position); // Position of error in query string
