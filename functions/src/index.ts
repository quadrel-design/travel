...
export const updateAllServicePricingScheduled = onSchedule(
  ...
  async (event) => {
    logger.info("Scheduled pricing update started for all services.", {
      event,
    });
    try {
      await updateAllPricing(); // Call the master update function
      logger.info(
        "Scheduled pricing update completed successfully for all services.",
      );
      // No explicit return needed on success for v2 onSchedule
    } catch (error) {
      const errorMessage =
        error instanceof Error ? error.message : String(error);
      logger.error("Scheduled pricing update failed for all services:", {
        error: errorMessage,
      });
...
export const testApiKeyConfig = onCall(
  ...
  async (request) => {
    // Explicitly type or ignore request data if unused
    ...
    const safeEnvVars = Object.keys(process.env).filter(
      (key) =>
...
export async function processScanImage(
  ...
  try {
    ...
    const analysisResult = await analyzeDetectedText(
      detectResult.extractedText,
    );
