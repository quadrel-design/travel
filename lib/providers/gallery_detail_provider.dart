...
class GalleryDetailState extends Equatable {
  ...
  GalleryDetailState copyWith({
    ...
    return GalleryDetailState(
      ...
      signedUrlError:
          clearSignedUrlError ? null : signedUrlError ?? this.signedUrlError,
      generalError: clearGeneralError
          ? null
          : generalError ?? this.generalError, // Handle new field
      scanningImageId: clearScanningImageId
          ? null
          : scanningImageId ??
              this.scanningImageId, // Handle scanningImageId, allow explicit clearing
    );
  }
...
class GalleryDetailNotifier extends StateNotifier<GalleryDetailState> {
  ...
  GalleryDetailNotifier(List<JourneyImageInfo> initialImages, this._logger,
      this._supabaseClient, Ref ref)
      : _imageIds = initialImages
            .map((img) => img.id)
        ...
        super(GalleryDetailState(
            ...
            signedUrls: List.filled(initialImages.length, null))) {
    _logger
        .i('GalleryDetailNotifier initialized for ${_imageIds.length} images.');
  ...
  String? _extractPath(String url) {
    try {
      ...
      final pathStartIndex =
          uri.path.indexOf(bucketName) + bucketName.length + 1;
  ...
  Future<void> _fetchSignedUrls() async {
    ...
    state = state.copyWith(
      ...
      signedUrlError: hadError ? 'gallery.signedUrlError' : null,
  ...
  void _setupRealtimeListener() {
    ...
    _channelName =
        'public:journey_images:detail_view_${DateTime.now().millisecondsSinceEpoch}_$hashCode';
    _logger.d(
        'Setting up Realtime listener for journey_images table using IN filter for ${_imageIds.length} IDs...');
    ...
    _subscription = _supabaseClient
        ...
        .listen(
          (List<Map<String, dynamic>> data) {
            _logger.d(
                'Realtime update received on channel $_channelName: ${data.length} item(s)');
          ...
          onError: (error) {
            _logger.e('Realtime listener error on channel $_channelName',
                error: error);
    ...
    _supabaseClient.channel(_channelName!).subscribe((status, [error]) {
      _logger.i(
          'GalleryDetailNotifier Realtime subscription status for $_channelName: $status');
      if (status == RealtimeSubscribeStatus.subscribed) {
        _logger.i(
            'GalleryDetailNotifier successfully subscribed to $_channelName.');
        // Clear general error and scanning state on successful sub
        state =
            state.copyWith(clearGeneralError: true, clearScanningImageId: true);
      } else if (status == RealtimeSubscribeStatus.closed ||
          status == RealtimeSubscribeStatus.channelError) {
        _logger.e('Realtime subscription failed/closed for $_channelName',
            error: error);
        ...
        // Set general error and clear scanning state
        state = state.copyWith(
            generalError: 'Subscription timed out', clearScanningImageId: true);
      }
    });
  }
  ...
  void _handleRealtimeUpdate(Map<String, dynamic> newRowData) {
    ...
    _logger.d(
        'Processing update for image ID: ${updatedImageInfo.id}, LastProcessed: ${updatedImageInfo.lastProcessedAt}');

    final index =
        state.images.indexWhere((img) => img.id == updatedImageInfo.id);
    if (index != -1) {
      ...
      if (clearScanning) {
        _logger.d(
            'Realtime update received for the image currently being scanned (${updatedImageInfo.id}), clearing scanning state.');
      }

      state = state.copyWith(
        images: updatedImages,
        scanError: newScanError,
        clearScanningImageId:
            clearScanning, // Clear scanning state if it matches
      );
    } else {
      _logger.w(
          'Received realtime update for unknown image ID: ${updatedImageInfo.id}');
    }
  }
  ...
  void handleDeletion(String deletedImageId) {
    ...
    if (currentImages.length < initialLength) {
      ...
      _logger.w(
          'Attempted to handle deletion for ID $deletedImageId, but it was not found in state.');
    }
  }
  ...
  void dispose() {
    _logger.d(
        'Disposing GalleryDetailNotifier. Unsubscribing from channel: $_channelName');
